...
static const UMI_LOC_NONE = 0;
static const UMI_LOC_INDEX1 = 1;
static const UMI_LOC_INDEX2 = 2;
static const UMI_LOC_READ1 = 3;
static const UMI_LOC_READ2 = 4;
static const UMI_LOC_PER_INDEX = 5;
static const UMI_LOC_PER_READ = 6;
...
public:
  ...
  bool validate() {
    ...
    if(correction.enabled && !isPaired()) {
      stderr.writeln("WARNING: base correction is only applicable for paired end data, ignored -c/--correction");
...
unittest{
  // Test a construct
  LowComplexityFilterOptions lcfo = LowComplexityFilterOptions() ;
  assert(0.3 == lcfo.threshold);

  // Test option class with default constructor
  Options myopts = new Options;
  assert(0 == myopts.trim.front1);
  // myopts.validate();
